...
export async function PATCH(request: Request) {
  ...
  if (data.requestId.length == 0 || data.newDecision.length == 0) {
  ...
  const requests = await ParkingSpotRequest.findOneAndUpdate(
    { _id: data.requestId },
    { decision: data.newDecision, lastInteractingAdminUserId: user?.id}
  );

  if (data.parkingSticker != undefined) {
    const requests = await ParkingSpotRequest.findOneAndUpdate(
      { _id: data.requestId },
      { parkingSticker: data.parkingSticker }
    );
  }
